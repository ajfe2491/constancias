        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div>
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('document-configurations.create', ['event_id' => $event->id]) }}" class="btn btn-primary btn-sm gap-2">
                ...
                @if($event->documentConfigurations->isEmpty())
                    <div class="bg-base-100 rounded-lg border-2 border-dashed border-base-300 p-12 text-center">
                        ...
                        <p class="opacity-60 mb-6">Crea plantillas específicas para este evento (ej. Constancia de Asistencia, Ponente, etc.)</p>
                        <a href="{{ route('document-configurations.create', ['event_id' => $event->id]) }}" class="btn btn-primary btn-sm">
                ...
                @else
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($event->documentConfigurations as $config)
                            <div class="card bg-base-100 shadow-md hover:shadow-lg transition-all duration-300 border border-base-200">
                                <div class="card-body p-4">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <h4 class="font-bold text-lg">{{ $config->document_name }}</h4>
                                            <p class="text-xs opacity-60 uppercase tracking-wide">{{ $config->document_type }}</p>
                                        </div>
                                        <div class="dropdown dropdown-end">
                                            ...
                                            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                                                ...
                                                <li>
                                                    <form action="{{ route('document-configurations.destroy', $config) }}" method="POST" onsubmit="return confirm('¿Estás seguro?');">
                                                        @csrf
                                                        @method('DELETE')
                                    ...
                                    <div class="mt-4 flex justify-end">
                                        <a href="{{ route('document-configurations.edit', $config) }}" class="btn btn-ghost btn-xs gap-1">
